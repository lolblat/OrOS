...
{
    if(size % KMALLOC_HEADER_LENGTH != 0) // align
    {
        size += KMALLOC_HEADER_LENGTH - (size % KMALLOC_HEADER_LENGTH);
    }
    ...
    while(m_page_addr + PAGE_SIZE <  *m_last_free_address + size + KMALLOC_HEADER_LENGTH) // need to Allocate page
    {
        m_allocator.AllocatePage();
        m_page_addr += PAGE_SIZE;
    }

    u32 free_space_after_alloc = *m_last_free_address - (size + KMALLOC_HEADER_LENGTH);
    ...
    *(u32*)(((u8*)m_last_free_address) + size + KMALLOC_HEADER_LENGTH + 4 ) = free_space_after_alloc;
    void *ret = (void*)((u8*)m_last_free_address + KMALLOC_HEADER_LENGTH);
    m_last_free_address = (u32*)((u32)m_last_free_address + size + KMALLOC_HEADER_LENGTH + 4);
